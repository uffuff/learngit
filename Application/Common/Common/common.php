...
function update_pay_status($order_sn,$pay_status = 1)
{
	if(stripos($order_sn,'recharge') !== false){
		...
		$order = M('order')->where("order_sn = '$order_sn'")->find();
                            ...
                            $erweima=buyerqrcode();
		// 修改支付状态  已支付
		M('order')->where("order_sn = '$order_sn'")->save(array('pay_status'=>1,'erweima'=>$erweima,'pay_time'=>time()));
		// 减少对应商品的库存
		...
		if($distribut_condition == 1)  // 购买商品付款才可以成为分销商
			M('users')->where("user_id = {$order['user_id']}")->save(array('is_distribut'=>1));
               ...
               function buyerqrcode()
               {
                 $ewmcode=get_rand_str('32','0','1');
                require_once 'ThinkPHP/Library/Vendor/phpqrcode/phpqrcode.php';
             $image = new \Think\Image();
              //import('Vendor.phpqrcode.phpqrcode'); 
              $errorCorrectionLevel = 'L';//容错级别   
              $matrixPointSize = 6;//生成图片大小 
               \QRcode::png('http://fanka.iruoyu.com/Mobile/User/usersent/token/'.$ewmcode,'./Uploads/user/'.$ewmcode.'.png',$errorCorrectionLevel,$matrixPointSize);
               $image->open('./Uploads/user/1212.jpg');
                $qrcode='/Uploads/user/'.$ewmcode.'.jpg';
                $image->water('./Uploads/user/'.$ewmcode.'.png',5,100)->save('.'.$qrcode);
                 $data['qrcode']=$qrcode;
                 $data['marketingcode']=$ewmcode;
                 return $ewmcode;   
               }
  ...
  /**
     * fanka订单确认收货
     * @param $erweima   订单派餐二维码32位编号
     */
    function fankaconfirm_order($erweima,$user_id = 0){
        
        $where = "erweima='$erweima'";
      // $user_id && $where .= " and user_id = $user_id ";
      $order = M('order')->where($where)->find();
       ...
       if($order['order_status'] == 2)
        ...
        $data['confirm_time'] = time(); // 收货确认时间
        $data['fankasent_userid']=$user_id;   //派餐人员userid号
        ...
        return array('status'=>1,'msg'=>'派餐成功','order_id'=>$order['order_id']);
    }
