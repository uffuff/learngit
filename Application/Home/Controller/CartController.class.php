...
class CartController extends BaseController {
    ...
    public function cart(){      
     ...
     //  $abc=$uffuff->where("user_id=1520")->delete();

     //   $abc1=$uffuff1->where("goods_id=134")->delete();
...
//多表事务
       $uffuff=M();
       $uffuff->startTrans();
       $abc=$uffuff->table('fktp_goods')->where("goods_id=133")->delete();
        $abc1=$uffuff->table('fktp_users')->where("user_id=1517")->delete();
       if($abc && $abc1)
       {
        $uffuff->commit();
       }    
 ...
 //超出个人积分提示     开始
         ...
         $where.=" and selected=1 ";
                          $cartList1= M('Cart')->where($where)->getField("id,goods_num,selected,prom_type,prom_id,jifen"); 
                               //print_r($cartList1);  
                                          foreach($cartList1 as $kk => $vv)
                                                      {
                                                                 
                                                             $zongjifen += (($cartList1[$kk]['jifen']*$cartList1[$kk]['goods_num'])/ tpCache('shopping.point_rate'));  
                                                                 
                                                      }
                                                     
           if($this->user['pay_points']<$zongjifen )
           {
            echo "<script>alert('积分不足');</script>";
             $result['total_price']['xiaofei_jifen']=$this->user['pay_points'];
           }  
           }         
       //超出个人积分提示     结束
...
//print_r($result);
        ...
        $this->assign('jifen_money', $result['total_price']['xiaofei_jifen']/tpCache('shopping.point_rate')); //  积分换算成消费金额       
   ...
   // print_r($result);
      if($this->user['pay_points']>=$result['total_price']['xiaofei_jifen'])
      {
           $xiaofei_jifen=$result['total_price']['xiaofei_jifen'];    
      }
      else
      {
        $xiaofei_jifen=$this->user['pay_points'];
      }
       //echo "xiaofei_jifen".$xiaofei_jifen;
        ...
        $this->assign('cartList', $result['cartList']); // 购物车的商品                
        $this->assign('total_price', $result['total_price']); // 总计  
        $this->assign('xiaofei_jifen', $xiaofei_jifen); // 消费积分                            
        $this->display();
    }
