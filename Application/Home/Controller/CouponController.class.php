...
class CouponController extends BaseController {
   ...
   $count =  M('coupon')->where(time()." >= send_start_time and ".time()." <= send_end_time ")->count();// 查询满足要求的总记录数
   echo $count;
        $Page = new \Think\Page($count,20);// 实例化分页类 传入总记录数和每页显示的记录数    
        $show = $Page->show();// 分页显示输出
        $this->assign('page',$show);// 赋值分页输出
        $list = M('coupon')->where(time()." >= send_start_time and ".time()." <= send_end_time ")->limit($Page->firstRow.','.$Page->listRows)->select(); // 找出这个商品        
