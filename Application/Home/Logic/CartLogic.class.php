  ...
  // print_r($goods);
      ...
      if($goods['exchange_integral']>0)
    {
     ...
     $user = session('user');
      ...
      $count_cart_jifen_list=M('Cart')->where($where)->select(); 

    foreach ($count_cart_jifen_list as $k=>$val){
           $cart_jifen = $val['goods_num'] * $val['jifen'];
                $zongjifen += ($cart_jifen / tpCache('shopping.point_rate'));             
        }

        
    $aaa=$goods['exchange_integral']+$zongjifen;
   if($user['pay_points']<$zongjifen ||  $user['pay_points']<$aaa)
   {
    return array('status'=>-005,'msg'=>'您的积分不够挥霍了亲!','result'=>'');
   }
   }}
       ...
       //    exit;
        ...
        $data = array(                    
                    ...
                    'jifen'                =>$goods['exchange_integral'],                   // 兑换所需积分          
        );                
    ...
    {                   
        ...
        foreach ($cartList as $k=>$val){
        	$cartList[$k]['goods_fee'] = $val['goods_num'] * $val['member_goods_price'];
             $cartList[$k]['goods_jifen'] = (($val['goods_num'] * $val['jifen'])/tpCache('shopping.point_rate'));
        	...
        	$total_price += $val['goods_num'] * $val['member_goods_price'];
               $total_jifen+=(($val['goods_num'] * $val['jifen'])/tpCache('shopping.point_rate'));
        }



//print_r($cartList);
        $total_price = array('total_fee' =>$total_price , 'cut_fee' => $cut_fee,'num'=> $anum,'xiaofei_jifen'=>$total_jifen); // 总计        
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
     ...
     * @param type $pickup_id 自提门店id
     * @return type $order_id 返回新增的订单id
     */
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$pickup_id)
    {
