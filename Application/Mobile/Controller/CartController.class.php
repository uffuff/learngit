...
class CartController extends MobileBaseController {
    ...
    public function cart2()
    {
        ...
        if(empty($address)){
          ...
          //获取全部自提点列表
                $getPickupList=$this->getPickupListByPCD();          
      //print_r($getPickupList);
  ...
  //增加自动填写积分结算  结束
        ...
        $this->assign('PickupList',$getPickupList);     //获取全部自提点列表
 ...
 /**
     ...
     * @return mixed
     */
    public function getPickupListByPCD()
    {
        $model = M('');
	...
	// 订单满额优惠活动		                
        ...
        {  
            ...
            $result = $this->cartLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price,$pickup_id); // 添加订单                        
            exit(json_encode($result));            
        }
                                          ...
                                          foreach($cartList1 as $kk => $vv)
                                                      {
                                                             ...
                                                             $zongjifen += $cartList1[$kk]['jifen']*$cartList1[$kk]['goods_num'];
