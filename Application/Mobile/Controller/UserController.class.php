...
//echo "今天的时间".strtotime(date('Y-m-d'));
$start=strtotime(date('Y-m-d'));
$now=time();
$end=strtotime(date('Y-m-d',strtotime('+1 day')));
    $user_appointment_time=$this->user['appointment_time'];
    $user_appointment_state=$this->user['appointment_state'];
    //print_r($this->user);
            if($start<$user_appointment_time && $user_appointment_time<$end || $user_appointment_state==1 )
            {
          $this->assign('prompt', '您已预约吃饭，谢谢你的大支持');

              //  $this->error('您已预约吃饭，谢谢你的大支持', U('User/points',array('type'=>points)));
                //exit;
            }
            else
            {
    ...
    $config = tpCache('basic');
     ...
     /* 更新用户信息 */
    $sql = "UPDATE  fktp_users SET pay_points = pay_points + {$config['appointment']}, appointment_time = {$data['appointment_time']},appointment_state=1 WHERE user_id = $this->user_id";

   $row=M('users')->execute($sql);
     if($row)
     {
          /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $this->user_id,
        'pay_points'    => $config['appointment'],
        'change_time'   => $now,
        'desc'   => '预约吃饭奖励'.$config['appointment'].'积分',
                                          );
    M('account_log')->add($account_log);
 // $this->success("收到您的预约，奖励{$config['appointment']}积分", U('User/points',array('type'=>points)));
        
         $this->assign('prompt', '感谢您的点到，本次准确点到将减少约1KG食物浪费，同时您也将获得'.$config['appointment'].'个猴币积分的奖励');
     }
          }
   $this->display();
...
$end=strtotime(date('Y-m-d',strtotime('+1 day')));
    ...
    $user_appointment_state=$this->user['appointment_state'];
      if($start<$user_appointment_time && $user_appointment_time<$end && $user_appointment_state==1)
            {
       //   $this->assign('prompt', '您已取消预约吃饭，谢谢你的大支持');
           $config = tpCache('basic');
         //  print_r($config);
     $data['appointment_time']=time();
     /* 更新用户信息 */
    $sql = "UPDATE  fktp_users SET pay_points = pay_points + {$config['cancel_reservation']}, appointment_time = {$data['appointment_time']},appointment_state=0 WHERE user_id = $this->user_id";
   ...
   $row=M('users')->execute($sql);
    ...
    $account_log = array(
        'user_id'       => $this->user_id,
        'pay_points'    => $config['cancel_reservation'],
        'change_time'   => $now,
        'desc'   => '取消预约吃饭奖励'.$config['cancel_reservation'].'积分',
                                          );
    M('account_log')->add($account_log);
 // $this->success("收到您的预约，奖励{$config['appointment']}积分", U('User/points',array('type'=>points)));
        
         $this->assign('prompt', '感谢您的取消点到，本次准确点到将减少约1KG食物浪费，同时您也将获得'.$config['cancel_reservation'].'个猴币积分的奖励');
     }
   ...
   //派餐列表
           public  function fankasentlist()
           {
      //$type = I('type','all');
     //   $this->assign('type',$type);
    
            $count = M('order')->where("fankasent_userid=" . $this->user_id)->count();
            $Page = new Page($count, 10);
            $account_log = M('order')->where("fankasent_userid=" . $this->user_id)->order('confirm_time desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
       ...
       //print_r($account_log);
        ...
        if ($_GET['is_ajax']) {
            $this->display('ajax_fankasentlist');
            exit;
        }
        $this->display();
           }
...
//派餐人员扫二维码到此方法处理
     public function usersent()
     {
         ...
         {
        if($this->user['fankasent']==1)
        {
      $token=I('get.token',0);
             if($token)
             {
             $fankastatus= fankaconfirm_order($token,$this->user_id); // 调用确认收货按钮
             //print_r($fankastatus);
                      if($fankastatus['status']==1)
                      {
                         $userLogic = new UsersLogic();
                         $s = $userLogic->orderActionLog($fankastatus['order_id'],'delivery','派餐人员ID：'.$this->user_id.'扫二维码完成派餐');//操作日志
                             if($s)
                             {
                                  $this->success("派餐成功", U('User/fankasentlist')); 
                             }
                      }
                        if($fankastatus['status']==-1)
                                      {
                                        $this->error($fankastatus['msg'], U('User/fankasentlist'));
                                      }
                                      if($fankastatus['status']==-3)
                                      {
                                         $this->error($fankastatus['msg'], U('User/fankasentlist'));
                                      }
