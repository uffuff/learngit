    ...
    <div class="layout">
    	<div class="bs-le">
        	<div class="first">
            	<ul>                 
                <?php
 $md5_key = md5("select * from `__PREFIX__goods` where is_new = 1 and is_hot = 1 and is_recommend = 1 and is_on_sale = 1 order by goods_id desc limit 4"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__goods` where is_new = 1 and is_hot = 1 and is_recommend = 1 and is_on_sale = 1 order by goods_id desc limit 4"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><li class="sellers">
                    	<div class="best">
    	...
    	<div class="bs-ri">
        	...
        	<div class="ris-notice">
            	...
            	<div class="notice_b">
                	<ul class="nob1">
                            <?php
 $md5_key = md5("select * from `__PREFIX__article`  where cat_id = 5 order by article_id desc limit 4"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__article`  where cat_id = 5 order by article_id desc limit 4"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><li><a href="<?php echo U('Home/Article/detail',array('article_id'=>$v[article_id]));?>"><?php echo (getSubstr($v['title'],0,18)); ?></a></li><?php endforeach; ?>    
                	</ul>
                    <ul class="nob2">
                            <?php
 $md5_key = md5("select * from `__PREFIX__article`  where cat_id = 6 order by article_id desc limit 4"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__article`  where cat_id = 6 order by article_id desc limit 4"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><li><a href="<?php echo U('Home/Article/detail',array('article_id'=>$v[article_id]));?>"><?php echo (getSubstr($v['title'],0,18)); ?></a></li><?php endforeach; ?>    
                     </ul>
                </div> 
             </div>
             <!--公告下方广告位-->
...
<!--------楼层-开始-------------->
<?php
 $md5_key = md5("select * from `__PREFIX__goods_category` where is_show = 1 and `level` = 1  limit 7"); $result_name = $sql_result_val = S("sql_".$md5_key); if(empty($sql_result_val)) { $Model = new \Think\Model(); $result_name = $sql_result_val = $Model->query("select * from `__PREFIX__goods_category` where is_show = 1 and `level` = 1  limit 7"); S("sql_".$md5_key,$sql_result_val,31104000); } foreach($sql_result_val as $key=>$val): $cat_id_arr = getCatGrandson($val[id]); $cat_id_str = implode(',',$cat_id_arr); ?>
	...
	<div class="layout layer">
    	<div class="ground-flool">
        	<div class="flool-t">
                ...
                <?php
 $md5_key = md5("select * from `__PREFIX__goods_category` where is_show = 1 and parent_id = $val[id]"); $result_name = $sql_result_val2 = S("sql_".$md5_key); if(empty($sql_result_val2)) { $Model = new \Think\Model(); $result_name = $sql_result_val2 = $Model->query("select * from `__PREFIX__goods_category` where is_show = 1 and parent_id = $val[id]"); S("sql_".$md5_key,$sql_result_val2,31104000); } foreach($sql_result_val2 as $key2=>$val2): ?><em><?php echo ($val2[name]); ?></em><?php endforeach; ?>                
                <ul><!--
        	...
        	<div class="flool-b layer">
            	<ul class="flool-cle">
            		<li class="sellers sell_ons">
                    	<div class="best">
                        <?php
 $md5_key = md5("select * from `__PREFIX__goods` where cat_id in($cat_id_str) and is_on_sale = 1 order by goods_id limit 1"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__goods` where cat_id in($cat_id_str) and is_on_sale = 1 order by goods_id limit 1"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><div class="best-about">
                    ...
                    <?php
 $md5_key = md5("select * from `__PREFIX__goods` where  cat_id in($cat_id_str) and is_on_sale = 1 order by goods_id limit 1,6"); $result_name = $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__goods` where  cat_id in($cat_id_str) and is_on_sale = 1 order by goods_id limit 1,6"); S("sql_".$md5_key,$sql_result_v,31104000); } foreach($sql_result_v as $k=>$v): ?><li class="sellers sellers2 sell_ons">
                            <div class="best">
                                <div class="best-about">
