	...
	<section id="filter_content">
	    ...
	    <ul class="filtrate_list" id="filter_category_list" style="display: block; -webkit-transform-origin: 0px 0px 0px; opacity: 1; -webkit-transform: scale(1, 1);">
	        <li class="filtrate_list_li">
				<ul>
				<?php if(is_array($cateArr)): foreach($cateArr as $k=>$v): ?><li class="filtrate_list_li"><a href="<?php echo U('Mobile/Goods/goodsList',array('id'=>$v['id']));?>" class="filtrate_list_li_a " style="padding-left:25px"><span><?php echo ($v["name"]); ?></span></a>
						<ul>
						<?php if(is_array($v["sub_menu"])): foreach($v["sub_menu"] as $key=>$vv): ?><li class="filtrate_list_li">
	     ...
	     <?php if(!empty($filter_brand)): ?><div class="filtrate_category" >
	       ...
	       <?php if(is_array($filter_spec)): foreach($filter_spec as $k=>$v): ?><div class="filtrate_category" >
	        ...
	        <ul class="clearfix filtrate_address filtrate_list att_item" id="attrs_<?php echo ($k); ?>">
		        <?php if(is_array($v[item])): foreach($v[item] as $k2=>$v2): ?><li id="attr_<?php echo ($k2); ?>">
		            <a  href="<?php echo ($v2[href]); ?>"  data-key='<?php echo ($v2[key]); ?>' data-val='<?php echo ($v2[val]); ?>'><?php echo ($v2[item]); ?></a>
		        </li><?php endforeach; endif; ?>
	        </ul><?php endforeach; endif; ?>
       
	      	<?php if(is_array($filter_attr)): foreach($filter_attr as $k=>$v): ?><div class="filtrate_category" >
		            <a href="javascript:;" class="filtrate_category_a"><?php echo ($v["attr_name"]); ?><span class="up_down">全部展开</span></a>
		        </div> 
		        <ul class="clearfix filtrate_address filtrate_list att_item" id="attrs_<?php echo ($k); ?>">
			        <?php if(is_array($v[attr_value])): foreach($v[attr_value] as $k2=>$v2): ?><li id="attr_<?php echo ($k2); ?>">
